...
# ─────────────── configuration ──────────────────────────────────────────────
TASK_FOLDER = "aesthetic_output"                            # ← change per script
OUTPUT_DIR  = (
...
def get_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        default=7.5,
        help="Classifier-free guidance scale (>=1) – larger = stronger prompt fidelity",
    )
...
def safe_name(prompt: str) -> str:
    """slug-ify prompt into a safe filename stem"""
...
def main() -> None:
    ...
    pipe = StableDiffusionPipeline.from_pretrained(
        ...
        torch_dtype=dtype,
        safety_checker=None,            # remove if you want NSFW filtering
    ).to(device)
